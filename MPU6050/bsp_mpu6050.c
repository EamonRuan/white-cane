...
// 阈值定义（需根据实际实验调整）
#define FALL_PITCH_THRESHOLD  60.0  // 俯仰角阈值（度）
#define FALL_ROLL_THRESHOLD   45.0  // 横滚角阈值（度）
#define FALL_TIME_THRESHOLD   1000  // 持续时间阈值（毫秒）
float X;
float Y;

float pitch,roll,yaw;
uint8_t fall_flag;
uint8_t orientation_initialized = 0;

void fall_GetStatus()
    {
            DL_TimerG_setTimerCount(TIMER_MPU6050_INST, 0);
            ...
            status = mpu_dmp_get_data(&pitch,&roll,&yaw);
             ...
             if(!orientation_initialized)
             {
                ...
                if( status == 0 )
                { 
                if(X-FALL_PITCH_THRESHOLD>pitch || X+FALL_PITCH_THRESHOLD<pitch ||
                    Y-FALL_ROLL_THRESHOLD>roll || Y+FALL_ROLL_THRESHOLD<roll )
                    {
